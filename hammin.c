...
uint16_t ham_encode(uint16_t in){
	...
	for(int i=0; i<4; ++i){
		...
		uint16_t left_mask = (~0u) << offset;
		uint16_t left = cor & left_mask;

		uint16_t right_mask = ~left_mask;
		uint16_t right = cor & right_mask;
...
uint16_t ham_decode(uint16_t in){
	...
	for(int i=0; i<4; ++i){
		...
		uint32_t left_mask = (~0u) << (offset+1-i);
		uint16_t left = (cor & left_mask) >> 1;

		uint16_t right_mask = (~left_mask) >> 1;
		uint16_t right = cor & right_mask;
